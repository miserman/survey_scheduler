...
var express = require('express'),
  ...
  message,
  messageIds = {},
  patterns = {colon: /:/g, slash: /\//g},
  report_record = {from: 0, scheduled: -1},
  nthscan = 100,
  ...
  beeps = {},
  held = {},
  logs = {enabled: true, dir: 'var/log/scheduler'},
  cookie_options = {
    signed: true,
    secure: true,
    httpOnly: true,
...
http
  .get(process.env.ISSUER + '/.well-known/jwks.json', function (res) {
    res.on('data', function (d) {
      keys.push(d)
    })
    res.on('end', function () {
      keys = JSON.parse(keys.join('')).keys
    })
  })
  .end()
function verify_jwt(token) {
  var i = keys.length,
    r,
    t = jwt.decode(token, {complete: true}),
    p
  if (t && t.header && t.header.alg === 'RS256') {
    ...
    if (
      p.exp > Date.now() / 1e3 &&
      p.token_use === 'access' &&
      p.iss === process.env.ISSUER &&
      p.client_id === process.env.CLIENT
    ) {
...
function log(s, entry) {
  var date = date_formatter.format(new Date()).split(', '),
    m = s + '_' + date[0].replace(patterns.slash, '') + '.txt'
  console.log(m + ' - ' + date[1] + ': ' + entry)
  if (logs.enabled)
    fs.appendFile(logs.dir + '/' + m, date[1] + ': ' + entry + '\n', function (e) {
    ...
    if (message_meta) {
      ...
      if (exists) {
        ...
        if (Object.hasOwn(mo, 'messages')) {
          ...
          m = ', ' + l + 'messages = :m'
          req.ExpressionAttributeValues[':m'] = []
          for (i = studies[s].participants[id].schedule[day].statuses.length; i--; )
            req.ExpressionAttributeValues[':m'].push({})
...
function send_message(s, id, day, index, status) {
  if (time_exists(s, id, day, index)) {
    var now = Date.now(),
      to = s + id,
      p = studies[s].participants[id],
      ds = p.schedule[day],
      pr = studies[s].protocols[ds.protocol],
    ...
    if (!pr) {
      console.log(
        'message for ' +
          ...
          '][' +
          index +
          '] not sent because protocol ' +
          ds.protocol +
          ' was not found in study ' +
          s
      )
      return
    }
    if (ds.statuses[index] === 6) {
      ...
      message.publish(
        {
          Message:
            pr[status === 3 ? 'reminder_message' : 'initial_message'] +
            (pr.link
              ? ' ' +
                (status !== 3 || pr.reminder_link
                  ? pr.link.replace(/^https?:\/\//, '') + (/\?/.test(pr.link) ? '&' : '?') + pr.id_parameter + '=' + id
                  : '')
              : ''),
        ...
        function (e, d) {
          var m =
              'send ' + (status === 3 ? 'reminder' : 'initial') + ' text to ' + id + '[' + day + '][' + index + ']: ',
            t,
            now = Date.now()
          if (e) {
            ...
            if (status === 2) {
              if (ds.accessed_n[index] === 0 && pr.remind_after && ds.times[index] + pr.remind_after * 6e4 > now) {
                beeps[s + id + day + index] = setTimeout(
                  send_message.bind(null, s, id, day, index, 3),
      ...
      log(
        s,
        'skipped sending scheduled beep (' +
          id +
          '[' +
          day +
          '][' +
...
function schedule(s, id, day) {
  var to = s + id,
    d = studies[s].participants[id].schedule[day],
    pr = studies[s].protocols[d.protocol],
    t,
    minsep,
    remind,
    end,
  ...
  if (!d) {
    ...
    for (n = d.times.length, i = 0; i < n; i++) {
      ...
      if (
        t - now < 3e8 &&
        (d.statuses[i] === 6 || d.statuses[i] === 1 || (d.statuses[i] === 2 && pr.remind_after && t + remind < now))
      ) {
        nt = 0
        if (d.statuses[i] === 6) {
          if (t < now) {
          ...
          t > now ||
          (now < (nt = i === n - 1 ? (end < t ? end + 864e5 : end) : d.times[i + 1] - minsep) &&
            (!pr.close_after || t + pr.close_after * 6e4 > now) &&
            (d.statuses[i] === 1 ||
          ...
          if (nt)
            log(
              ...
              'beep ' +
                ...
                '][' +
                i +
                '] sent retroactively; scheduled for ' +
                t +
                ', send at ' +
                Date.now() +
                ', before ' +
          ...
          any = true
          beeps[s + id + day + i] = setTimeout(
            send_message.bind(null, s, id, day, i, d.statuses[i] === 1 ? 2 : 3),
            t - Date.now()
          )
    ...
    console.log(
      s + ' ' + id + ': day ' + day + (pr ? ' has no times: ' : ' has protocol ' + d.protocol + 'which is not found: '),
      d
    )
  return {any: any, nmisses: nmisses, misses: misses}
}
function clear_schedule(s, id) {
  var p = studies[s].participants[id].schedule,
    d,
    i,
    sid = ''
  for (d = p.length; d--; )
    for (i = p[d].times.length; i--; )
      if ('undefined' !== beeps[(sid = s + id + d + i)]) {
...
function scan_database(s) {
  database.scan({TableName: s, Select: 'ALL_ATTRIBUTES'}, function (e, d) {
    var i,
      m = 'download ' + s + ' participants database: ',
      day,
      any,
      anyany,
      res,
  ...
  if (Object.hasOwn(studies, s)) {
    if (Object.hasOwn(studies[s], 'participants')) {
      ...
      if (nmissing[0])
        email_notification(
          'Missed Beeps in ' + s,
          (nmissing[1] === 1 ? 'A beep was' : 'Beeps were') +
            ' missed for ' +
            (nmissing[0] === 1 ? 'a participant' : 'participants') +
            ' in ' +
            s +
            ' (participant: day[times]):\n' +
            misses
        )
    }
  }
...
function scan_local(report) {
              ...
              for (nd = day.statuses.length, i = 0; i < nd; i++) {
                ...
                if (day.times[i] < now) {
                  if (
                    ...
                    pr &&
                    pr.remind_after &&
                    day.times[i] + pr.remind_after * 6e4 < now &&
                    timeiw
                  ) {
                    ...
                    report &&
                    day.times[i] > report_record.from &&
                    day.statuses[i] > 1 &&
                    day.statuses[i] < 8
                  ) {
  ...
  if (n)
    email_notification(
      subj,
      (caught.length === 1 ? 'A beep was' : 'Some beeps were') +
        ...
        for (participant in events[study])
          if (Object.hasOwn(events[study], participant)) {
            ...
            body +=
              ...
              participant +
              ':' +
              (ps[0] ? ' Responded to ' + ps[7] + ' of ' + ps[0] + ' sent ' + (ps[0] === 1 ? 'beep.' : 'beeps.') : '') +
              (ps[4] ? ' ' + ps[4] + (ps[4] === 1 ? ' beep was' : ' beeps were') + ' skipped.' : '') +
              (ps[6] ? ' The delivery of ' + ps[6] + ' sent beep' + (ps[6] === 1 ? '' : 's') + ' failed.' : '')
          }
      }
    if (n)
      email_notification(
        'Status Report for ' + date.toLocaleDateString(),
        'Beeps were scheduled for ' +
          n +
          ' participants between ' +
          new Date(report_record.from).toString() +
          ' and ' +
...
function scan_for_report() {
  if (process.env.REPORT_HOUR > -1) {
    var now = Date.now(),
      to = new Date().setHours(process.env.REPORT_HOUR, 0, 0, 0) - now
...
app.post('/status', function (req, res) {
  if (req.body) {
    var body = {messageId: String(req.body.messageId)},
      cords = ['', '', 0, 0, 0],
      d,
      mtype
...
app.post('/checkin', function (req, res) {
  var r = {available: false, accessed: 0, days: 0, day: 0, first_of_day: true, beeps: 0, beep: 0},
    ...
    pdm,
    n = Date.now(),
    id = Sanitize.gen('id', req.body.id),
...
app.get('/auth', function (req, res) {
  if (
    ...
    req.query.state === req.signedCookies.id
  ) {
    var body =
...
app.post('/operation', function (req, res) {
  var id = req.signedCookies.id,
    nid = Sanitize.gen('id', req.body.id),
    name,
    type = 'none',
    check = {valid: false, expired: false, pass: false},
    s = Sanitize.gen('study', req.body.study),
    o,
    m
  if (id) {
    ...
    if (!Object.hasOwn(studies, s)) {
      ...
      switch (type) {
        case 'load_schedule':
          ...
          var k,
            r = {version: studies[s].version, participants: {}, protocols: studies[s].protocols, users: {}}
              ...
              database.delete(
                ...
                function (e, d) {
                  if (e) {
                    ...
                    o.status = /does not exists/.test(e.message)
                      ? 'study ' + s + ' does not exist'
                      : 'failed to remove study ' + s
                    res.status(400).json(o)
                  } else {
                    delete studies[s]
          ...
          database.delete(
            ...
            function (e, d) {
              if (e) {
                ...
                log(s, m + 'false')
                o.status = /does not exists/.test(e.message)
                  ? 'participant ' + nid + ' does not exist'
                  : 'failed to remove participant ' + nid
                res.status(400).json(o)
              } else {
                  ...
                  users.adminDeleteUser(
                    ...
                    function (e, d) {
                      ...
                      log(
                        s,
                        m +
                          (e ? (/does not exist/.test(e.message) ? 'false, account does not exist' : 'false') : 'true')
                      )
                    }
                  )
                }
